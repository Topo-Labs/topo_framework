# The Topo Framework Repo

This repository serves as a mirror for the Topo Framework packages, including the Move standard library. The contents are synced from [topo-chain](https://github.com/Topo-Labs/topo-chain.git) on an hourly basis.
...
```
[dependencies]
<package_name> = { git = "https://github.com/Topo-Labs/topo_framework.git", subdir = "<path_to_directory_containing_Move.toml>", rev = "<commit_hash_or_branch_name>" }
```
For example, to add `AptosFramework` from the `mainnet` branch, you would use:
```
AptosFramework = { git = "https://github.com/Topo-Labs/topo_framework.git", subdir = "topo-framework", rev = "mainnet" }
```
Make sure to replace `subdir` with the appropriate path if you are referencing a different package within the framework.

## Contributing
If you want to contribute to the development of the framework, please submit issues and pull requests to the [topo-chain](https://github.com/Topo-Labs/topo-chain.git) repository, where active development happens.
